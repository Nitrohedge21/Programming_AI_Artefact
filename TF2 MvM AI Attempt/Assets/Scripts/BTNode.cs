...
public class Selector : CompositeNode
{
    ...
    public override BTStatus Execute()
    {
        ...
        for (int i = CurrentChildIndex; i < children.Count; i++)
        {
            ...
            if(rv == BTStatus.SUCCESS) { Reset(); break; }

            if(rv == BTStatus.RUNNING) { CurrentChildIndex = i; break; }

            if(rv == BTStatus.FAILURE && children.Count -1 == i) { Reset(); }
        }
...
public class Sequence : CompositeNode
{
    ...
    public override BTStatus Execute()
    {
        ...
        for (int i = CurrentChildIndex; i < children.Count; i++)
        {
            ...
            if (rv == BTStatus.FAILURE) { Reset(); break; }

            if (rv == BTStatus.RUNNING) { CurrentChildIndex = i; break; }

            if (rv == BTStatus.SUCCESS && children.Count - 1 == i) { Reset(); }
        }
